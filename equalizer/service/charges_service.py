...
def get_threshold_spread_coef_for_reqd_profit(buy_value, profit_percent, product_type_int):
    kite_client = get_kite_client_from_cache()
    profit_coef = profit_percent / 100
    if product_type_int == kite_client.PRODUCT_CNC_INT:
        return ((15.93 + 0.002241 * buy_value) * (1 + profit_coef)) / buy_value + profit_coef
    if product_type_int == kite_client.PRODUCT_MIS_INT and buy_value > 66000.0:
